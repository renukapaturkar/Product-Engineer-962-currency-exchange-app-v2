...
2. **Secondary: ExchangeRate-API.io** - Generous usage (1,500/month) and requires your configured API key as the ultimate failsafe when Open Exchange Rates misconfigures.
...
2. If it times out or throws an authorization error, attempt the `ExchangeRate-API`.
3. If ALL live APIs collapse, we default to injecting stale server-side cache states.
4. Complete degradation returns an explicit "Systems Unavailable" to the end user.
...
Given the strict request limitations of 'Open Exchange Rates' (1,000/month):
